...
(Gladstone activation, Replogle essential-gene, Norman). Uses the LOCAL norman_effects.npz (no download).
...
Run: pixi run python src/eval/run_parity_norman.py
"""
import os
import sys
...
EFFECTS = "results/features/norman_effects.npz"
OUT = "results/parity"
SEEDS = [42, 43, 44]
N_TOP = 1000
N_FOLDS = 5
...
def main():
    ...
    agg.to_csv(os.path.join(OUT, "parity_norman.csv"), index=False)
    print("=== D3/D5 parity on Norman K562 single-gene (Ahlmann-Eltze metrics) ===")
    ...
    print(f"wrote {OUT}/parity_norman.csv")
